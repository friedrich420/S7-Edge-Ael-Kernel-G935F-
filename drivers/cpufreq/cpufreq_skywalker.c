 ...
 * Edited: Tegrak (luciferanna@gmail.com); Erik Birgiser (ariesrik96@gmail.com); John Thacher (john_thatcher@ymail.com)
 *
 * Driver values in /sys/devices/system/cpu/cpufreq/skywalker
 *
 */
...
#define SKYWALKER_VERSION	(1)
#define SKYWALKER_AUTHOR	"JOHN"

// if you changed some codes for optimization, just write your name here.
#define SKYWALKER_TUNER "DerTeufel1980 - CastagnaIT - Christopher83 - erik96"

#define LOGI(fmt...) printk(KERN_INFO "[skywalker] " fmt)
#define LOGW(fmt...) printk(KERN_WARNING "[skywalker] " fmt)
#define LOGD(fmt...) printk(KERN_DEBUG "[skywalker] " fmt)

static void (*pm_idle_old)(void);
static atomic_t active_count = ATOMIC_INIT(0);

struct cpufreq_skywalker_cpuinfo {
...
static DEFINE_PER_CPU(struct cpufreq_skywalker_cpuinfo, cpuinfo);
 ...
 */
#define DEFAULT_UP_SAMPLE_TIME 11000 // Scale directly to the max, reduce waiting time to 10ms
 ...
 */
#define DEFAULT_DOWN_SAMPLE_TIME 22000
...
enum {
	SKYWALKER_DEBUG_EARLY_SUSPEND=1,
	SKYWALKER_DEBUG_START_STOP=2,
	SKYWALKER_DEBUG_LOAD=4,
	SKYWALKER_DEBUG_SUSPEND=8,
};
...
Skywalker - as max_up_step is present, the CPU must scale up to the max, so the loading measuring is decreased.*/
 ...
 * not implemented yet.
 */
#define DEFAULT_DEC_CPU_LOAD 65
...
struct cpufreq_governor cpufreq_gov_skywalker = {
	...
	.max_transition_latency = 8500000,
	.owner = THIS_MODULE,
};
...
static inline void fix_screen_off_min_step(struct cpufreq_skywalker_cpuinfo *pcpu) {
...
static inline unsigned int adjust_screen_off_freq(
                                                  struct cpufreq_skywalker_cpuinfo *pcpu, unsigned int freq) {
...
static void cpufreq_skywalker_timer(unsigned long data)
{
	...
	u64 time_in_idle;
	u64 idle_exit_time;
	struct cpufreq_skywalker_cpuinfo *pcpu =
	...
	/*
	 * START skywalker algorithm section
	 */
	if (early_suspended) {
		...
		if (max_up_step && pcpu->policy->cur < pcpu->policy->max) {
			...
			// apply max_up_step - modified for Skywalker
			index += max_up_step;
			if (index < 0)
				index = 0;
	...
	if (suspending && (debug_mode & SKYWALKER_DEBUG_SUSPEND)) {
		LOGI("suspending: cpu_load=%d%% new_freq=%u ppcpu->policy->cur=%u\n",
			 cpu_load, new_freq, pcpu->policy->cur);
	}
	if (early_suspended && !suspending && (debug_mode & SKYWALKER_DEBUG_LOAD)) {
		LOGI("early_suspended: cpu_load=%d%% new_freq=%u ppcpu->policy->cur=%u\n",
			 cpu_load, new_freq, pcpu->policy->cur);
	}
	if ((debug_mode & SKYWALKER_DEBUG_LOAD) && !early_suspended && !suspending) {
...
static void cpufreq_skywalker_idle(void)
{
	struct cpufreq_skywalker_cpuinfo *pcpu =
...
static int cpufreq_skywalker_up_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	struct cpufreq_skywalker_cpuinfo *pcpu;
...
static void cpufreq_skywalker_freq_down(struct work_struct *work)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	struct cpufreq_skywalker_cpuinfo *pcpu;
                                       ...
                                       unsigned int event)
{
	...
	switch (event) {
        case CPUFREQ_GOV_START:
            if (debug_mode & SKYWALKER_DEBUG_START_STOP) {
            ...
            //pm_idle_old = pm_idle;
            //pm_idle = cpufreq_skywalker_idle;
            break;
...
static int skywalker_pm_notifier_event(struct notifier_block *this,
                                        unsigned long event, void *ptr)
{
	...
	switch (event) {
        case PM_SUSPEND_PREPARE:
            suspending = 1;
            if (debug_mode & SKYWALKER_DEBUG_SUSPEND) {
...
static struct notifier_block skywalker_pm_notifier = {
	.notifier_call = skywalker_pm_notifier_event,
};
...
static int __init cpufreq_skywalker_init(void)
{
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_skywalker_timer;
		pcpu->cpu_timer.data = i;
	}

	up_task = kthread_create(cpufreq_skywalker_up_task, NULL,
                             "kskywalkerup");
	if (IS_ERR(up_task))
	...
	register_pm_notifier(&skywalker_pm_notifier);
...
fs_initcall(cpufreq_skywalker_init);
#else
...
static void __exit cpufreq_skywalker_exit(void)
{
	...
	unregister_early_suspend(&skywalker_power_suspend);
	unregister_pm_notifier(&skywalker_pm_notifier);
...
MODULE_AUTHOR("Tegrak <luciferanna@gmail.com>; Erik Birgiser <ariesrik96@gmail.com>; John Thatcher <john_thatcher@ymail.com>");
MODULE_DESCRIPTION("'Skywalker' - a conbination of Lulzactive and Performance");
MODULE_LICENSE("GPL");
