/*
 * drivers/cpufreq/cpufreq_lulzactivew.c
 *
 ...
 * Edited: Tegrak (luciferanna@gmail.com); Erik Birgiser (ariesrik96@gmail.com)
 *
 * Driver values in /sys/devices/system/cpu/cpufreq/lulzactivew
 *
 */
...
#define LULZACTIVEW_TUNER "DerTeufel1980 - CastagnaIT - Christopher83 - erik96"

#define LOGI(fmt...) printk(KERN_INFO "[lulzactivew] " fmt)
#define LOGW(fmt...) printk(KERN_WARNING "[lulzactivew] " fmt)
#define LOGD(fmt...) printk(KERN_DEBUG "[lulzactivew] " fmt)

static void (*pm_idle_old)(void);
static atomic_t active_count = ATOMIC_INIT(0);

struct cpufreq_lulzactivew_cpuinfo {
...
static DEFINE_PER_CPU(struct cpufreq_lulzactivew_cpuinfo, cpuinfo);
...
enum {
	LULZACTIVEW_DEBUG_EARLY_SUSPEND=1,
	LULZACTIVEW_DEBUG_START_STOP=2,
	LULZACTIVEW_DEBUG_LOAD=4,
	LULZACTIVEW_DEBUG_SUSPEND=8,
};
...
/*
 * Increase CPU freq to max - LulzactiveW
 */
#define DEFAULT_MAX_UP_STEP 0
static unsigned long max_up_step;
...
#define DEFAULT_SCREEN_OFF_MIN_STEP 5
...
static int cpufreq_governor_lulzactivew(struct cpufreq_policy *policy,
                                       unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_LULZACTIVEW
static
#endif
...
static inline void fix_screen_off_min_step(struct cpufreq_lulzactivew_cpuinfo *pcpu) {
...
static inline unsigned int adjust_screen_off_freq(
                                                  struct cpufreq_lulzactivew_cpuinfo *pcpu, unsigned int freq) {
...
static void cpufreq_lulzactivew_timer(unsigned long data)
{
	...
	u64 time_in_idle;
	u64 idle_exit_time;
	struct cpufreq_lulzactivew_cpuinfo *pcpu =
	...
	/*
	 * START lulzactivew algorithm section
	 */
	if (early_suspended) {
		...
		if (max_up_step && pcpu->policy->cur < pcpu->policy->max) {
			...
			// apply max_up_step - modified for LulzactiveW
			index += max_up_step;
			if (index < 0)
				index = 0;
	...
	if (suspending && (debug_mode & LULZACTIVEW_DEBUG_SUSPEND)) {
		LOGI("suspending: cpu_load=%d%% new_freq=%u ppcpu->policy->cur=%u\n",
			 cpu_load, new_freq, pcpu->policy->cur);
	}
	if (early_suspended && !suspending && (debug_mode & LULZACTIVEW_DEBUG_LOAD)) {
		LOGI("early_suspended: cpu_load=%d%% new_freq=%u ppcpu->policy->cur=%u\n",
			 cpu_load, new_freq, pcpu->policy->cur);
	}
	if ((debug_mode & LULZACTIVEW_DEBUG_LOAD) && !early_suspended && !suspending) {
...
static void cpufreq_lulzactivew_idle(void)
{
	struct cpufreq_lulzactivew_cpuinfo *pcpu =
...
static int cpufreq_lulzactivew_up_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	struct cpufreq_lulzactivew_cpuinfo *pcpu;
...
static void cpufreq_lulzactivew_freq_down(struct work_struct *work)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	struct cpufreq_lulzactivew_cpuinfo *pcpu;
...
static struct global_attr inc_cpu_load_attr = __ATTR(inc_cpu_load, 0660,
...
static struct global_attr dec_cpu_load_attr = __ATTR(dec_cpu_load, 0660,
...
static struct global_attr down_sample_time_attr = __ATTR(down_sample_time, 0660,
...
static struct global_attr up_sample_time_attr = __ATTR(up_sample_time, 0660,
...
static struct global_attr debug_mode_attr = __ATTR(debug_mode, 0660,
                                                   show_debug_mode, store_debug_mode);

// max_up_step
static ssize_t show_max_up_step(struct kobject *kobj,
                                 struct attribute *attr, char *buf)
{
	return sprintf(buf, "%lu\n", max_up_step);
}

static ssize_t store_max_up_step(struct kobject *kobj,
                                  struct attribute *attr, const char *buf, size_t count)
{
	if(strict_strtoul(buf, 0, &max_up_step)==-EINVAL) return -EINVAL;
	return count;
}

static struct global_attr max_up_step_attr = __ATTR(max_up_step, 0660,
                                                     show_max_up_step, store_max_up_step);
...
static struct global_attr pump_down_step_attr = __ATTR(pump_down_step, 0660,
...
static struct global_attr screen_off_min_step_attr = __ATTR(screen_off_min_step, 0660,
...
static struct attribute_group lulzactivew_attr_group = {
	.attrs = lulzactivew_attributes,
	.name = "lulzactiveW",
};

static int cpufreq_governor_lulzactivew(struct cpufreq_policy *new_policy,
                                       unsigned int event)
{
	int rc;
	struct cpufreq_lulzactivew_cpuinfo *pcpu =
	...
	switch (event) {
        case CPUFREQ_GOV_START:
            if (debug_mode & LULZACTIVEW_DEBUG_START_STOP) {
...
static void lulzactivew_early_suspend(struct early_suspend *handler) {
	struct cpufreq_lulzactivew_cpuinfo *pcpu;
...
static void lulzactivew_late_resume(struct early_suspend *handler) {
	early_suspended = 0;
...
static int lulzactivew_pm_notifier_event(struct notifier_block *this,
                                        unsigned long event, void *ptr)
{
	...
	switch (event) {
        case PM_SUSPEND_PREPARE:
            suspending = 1;
            if (debug_mode & LULZACTIVEW_DEBUG_SUSPEND) {
...
static struct notifier_block lulzactivew_pm_notifier = {
	.notifier_call = lulzactivew_pm_notifier_event,
};

static int __init cpufreq_lulzactivew_init(void)
{
	unsigned int i;
	struct cpufreq_lulzactivew_cpuinfo *pcpu;
	...
	max_up_step = DEFAULT_MAX_UP_STEP;
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_lulzactivew_timer;
		pcpu->cpu_timer.data = i;
	}

	up_task = kthread_create(cpufreq_lulzactivew_up_task, NULL,
                             "klulzactivewup");
	if (IS_ERR(up_task))
	...
	down_wq = create_workqueue("klulzactivew_down");

	if (! down_wq)
		goto err_freeuptask;

	INIT_WORK(&freq_scale_down_work,
              cpufreq_lulzactivew_freq_down);
	...
	register_pm_notifier(&lulzactivew_pm_notifier);
	register_early_suspend(&lulzactivew_power_suspend);

	return cpufreq_register_governor(&cpufreq_gov_lulzactivew);
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_LULZACTIVEW
fs_initcall(cpufreq_lulzactivew_init);
#else
module_init(cpufreq_lulzactivew_init);
#endif

static void __exit cpufreq_lulzactivew_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_lulzactivew);
	unregister_early_suspend(&lulzactivew_power_suspend);
	unregister_pm_notifier(&lulzactivew_pm_notifier);
	kthread_stop(up_task);
	put_task_struct(up_task);
	destroy_workqueue(down_wq);
}

module_exit(cpufreq_lulzactivew_exit);
